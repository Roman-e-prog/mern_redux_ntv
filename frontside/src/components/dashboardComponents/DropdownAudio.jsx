...
import { createDropdownAudio, getAllDropdownAudio, deleteDropdownAudio, reset } from "../../features/dropdown/audioSlice";
...
const DropdownAudio = () => {
  ...
  const dropdownAudios = useSelector((state)=>state.dropdownAudio.dropdownAudios);
  const isLoading = useSelector((state)=>state.dropdownAudio.isLoading);
  const isError = useSelector((state)=>state.dropdownAudio.isError);
  const message = useSelector((state)=>state.dropdownAudio.message);
  ...
  const [formdata, setFormdata] = useState(
    ...
    const onSubmit = async (e)=>{
      ...
      const dropdownAudioData = new FormData();
      dropdownAudioData.append("lis",JSON.stringify(formdata.lis));
      dropdownAudioData.append("themen",JSON.stringify(formdata.themen));
      dropdownAudioData.append("videos",JSON.stringify(formdata.videos));
      filedata.iframe.forEach((file, index) => {
        dropdownAudioData.append(`iframe${index}`, file);
      });    
      await dispatch(createDropdownAudio(dropdownAudioData));
      dispatch(getAllDropdownAudio());
    }
    else{
    ...
    const handleDelete = async (id)=>{
     await dispatch(deleteDropdownAudio(id));
      dispatch(getAllDropdownAudio());
    }
    if(isLoading){
      return <Spinner/>
  ...
  return (
    <Container>
      <ToastContainer/>
            <CrudTitleHolder>
              <CrudTitle>Dropdown Menü Audio</CrudTitle>
            </CrudTitleHolder>
            <DataWrapper>
            {dropdownAudios.length > 0 && <DataHolder>
                {dropdownAudios.map((item, index)=>(
                  <div key={index}>
                  <h4 id="name">{item.name}</h4>
                      ...
                      <VideoWrapper>
                        ...
                        <DataButtonHolder>
                      <DataUpdateButton><Link to={`/dropdownaudioedit/${item._id}`} className="link" style={{color:"white"}}>Update</Link></DataUpdateButton>
            ...
            <Form onSubmit={onSubmit}>
                <FormGroup className="menupoints">
                  <Label htmlFor={`${uniqueId} lis`}>Menüpunkte Audio</Label>
                ...
                <Videos onChange={(e)=>handleVideoChange(0,e)}>
                    ...
                    <Videos onChange={(e)=>handleVideoChange(1,e)}>
                      ...
                      <Videos onChange={(e)=>handleVideoChange(2,e)}>
                        ...
                        <FormGroup>
                          <Label htmlFor={`${uniqueId} title`}>Video Titel</Label>
                          <Input type="text" name="title" id={`${uniqueId} title`} value={title}/>
                          <div className='error'>
