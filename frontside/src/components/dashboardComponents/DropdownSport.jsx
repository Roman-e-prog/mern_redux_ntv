...
import { createDropdownSport, 
          getAllDropdownSport, 
          deleteDropdownSport, 
          reset } from "../../features/dropdown/sportSlice";
...
const DropdownSport = () => {
  ...
  const {dropdownSport, isLoading, isError, message} = useSelector((state)=>state.dropdownSport);
...
//bring out
  const [formdata, setFormdata] = useState(
    ...
    const onSubmit = async (e)=>{
      ...
      const dropdownSportData = new FormData();
      dropdownSportData.append("lis",JSON.stringify(formdata.lis));
      dropdownSportData.append("themen",JSON.stringify(formdata.themen));
      dropdownSportData.append("videos",JSON.stringify(formdata.videos));
      filedata.iframe.forEach(( file, index)=>{
        dropdownSportData.append(`iframe${index}`, file);
      })
      await dispatch(createDropdownSport(dropdownSportData));
      dispatch(getAllDropdownSport());
    }
    else{
    ...
    const handleDelete = async (id)=>{
      await dispatch(deleteDropdownSport(id));
      dispatch(getAllDropdownSport());
    }
    if(isLoading){
      return <Spinner/>
  ...
  return (
    <Container>
      <ToastContainer/>
            <CrudTitleHolder>
              <CrudTitle>Dropdown Menü Sport</CrudTitle>
            </CrudTitleHolder>
            <DataWrapper>
            {dropdownSport.length > 0 && <DataHolder>
                {dropdownSport.map((item, index)=>(
                  <div key={index}>
                    ...
                    <DataButtonHolder>
                      <DataUpdateButton><Link to={`/dropdownsportedit/${item._id}`} className="link" style={{color:"white"}}>Update</Link></DataUpdateButton>
