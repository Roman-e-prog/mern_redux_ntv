...
import { createDropdownVideos, 
        getAllDropdownVideos, 
        deleteDropdownVideos, 
        reset } from "../../features/dropdown/videosSlice";
...
const DropdownVideo = () => {
  ...
  const {dropdownVideos, isLoading, isError, message} = useSelector((state)=>state.dropdownVideos);
  ...
  const [formdata, setFormdata] = useState(
    ...
    const onSubmit = async (e)=>{
      ...
      const dropdownVideoData = new FormData();
      dropdownVideoData.append("lis",JSON.stringify(formdata.lis));
      dropdownVideoData.append("themen",JSON.stringify(formdata.themen));
      dropdownVideoData.append("videos",JSON.stringify(formdata.videos));
      filedata.iframe.forEach((file, index)=>{
        dropdownVideoData.append(`iframe${index}`, file);
      })
     await dispatch(createDropdownVideos(dropdownVideoData));
      dispatch(getAllDropdownVideos());
    }   
    else{
    ...
    const handleDelete = async (id)=>{
     await dispatch(deleteDropdownVideos(id));
      dispatch(getAllDropdownVideos());
    }
    if(isLoading){
      return <Spinner/>
  ...
  return (
    <Container>
      <ToastContainer/>
            <CrudTitleHolder>
              <CrudTitle>Dropdown Menü Videos</CrudTitle>
            </CrudTitleHolder>
            <DataWrapper>
            {dropdownVideos.length > 0 && <DataHolder>
                {dropdownVideos.map((item, index)=>(
                  <div key={index}>
                    ...
                    <DataButtonHolder>     
                      <DataUpdateButton><Link to={`/dropdownvideoedit/${item._id}`} className="link" style={{color:"white"}}> Update</Link></DataUpdateButton>
            ...
            <Form onSubmit={onSubmit}>
                <FormGroup className="menupoints">
                  <Label htmlFor={`${uniqueId} lis`}>Menüpunkte Video</Label>
