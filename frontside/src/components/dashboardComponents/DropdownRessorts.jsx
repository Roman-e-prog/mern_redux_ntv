...
import { createDropdownRessort , getAllDropdownRessort, deleteDropdownRessort, reset} from "../../features/dropdown/ressortSlice";
...
//bring out
  const [formdata, setFormdata] = useState(
    ...
    const onSubmit = async (e) => {
      ...
      const ressortdata = new FormData();
      ressortdata.append("lis", JSON.stringify(formdata.lis));
      ressortdata.append("themen", JSON.stringify(formdata.themen));
      ressortdata.append("videos", JSON.stringify(formdata.videos));
      filedata.iframe.forEach((file, index) => {
        ressortdata.append(`iframe${index}`, file);
      });
      await dispatch(createDropdownRessort(ressortdata));
       dispatch(getAllDropdownRessort())
    }
    else{
    ...
    const handleDelete = async (id)=>{
      await dispatch(deleteDropdownRessort(id))
      dispatch(getAllDropdownRessort());
    }
    if(isLoading){
      return <Spinner/>
    ...
    <Container>
      <ToastContainer/>
            ...
            {dropdownRessort.length > 0 && <DataHolder>
                {dropdownRessort.length > 0 && dropdownRessort.map((item, index)=>(
                  <div key={index}>
                      ...
                      <VideoWrapper>
                        ...
                        <Themen>Themen: {item.themen}</Themen>
                    <DataButtonHolder>
                      <DataUpdateButton><Link to={`/dropdownressortedit/${item._id}`} className="link" style={{color:"white"}}>Update</Link></DataUpdateButton>
            ...
            <Form onSubmit={onSubmit}>
                <FormGroup className="menupoints">
                  <Label htmlFor={`${uniqueId} lis`}>Menüpunkte Ressorts</Label>
