...
import { createDropdownProgramm, 
          getAllDropdownProgramm, 
          deleteDropdownProgramm, 
          reset } from "../../features/dropdown/programmSlice"; 
...
const DropdownProgramm = () => {
  //bring in
  const dispatch = useDispatch();
  const {dropdownProgramm, isLoading, isError, message} = useSelector((state)=>state.dropdownProgramm);
...
//bring out
  const [formdata, setFormdata] = useState(
    ...
    const onSubmit = async (e)=>{
      ...
      dropdownProgrammData.append("themen",JSON.stringify(formdata.themen));
      dropdownProgrammData.append("videos",JSON.stringify(formdata.videos));
      filedata.iframe.forEach((file, index)=>{
        dropdownProgrammData.append(`iframe${index}`, file);
      })
      await dispatch(createDropdownProgramm(dropdownProgrammData))
      dispatch(getAllDropdownProgramm());
    }
    else{
    ...
    const handleDelete = async (id)=>{
      await dispatch(deleteDropdownProgramm(id))
      dispatch(getAllDropdownProgramm());
    }
    if(isLoading){
      return <Spinner/>
  ...
  return (
    <Container>
      <ToastContainer/>
            <CrudTitleHolder>
              <CrudTitle>Dropdown Menü Programm</CrudTitle>
            </CrudTitleHolder>
            <DataWrapper>
            {dropdownProgramm.length > 0 && <DataHolder>
                {dropdownProgramm.map((item, index)=>(
                  <div key={index}>
                      ...
                      <VideoWrapper>
                        ...
                        <DataButtonHolder>
                      <DataUpdateButton><Link to={`/dropdownprogrammedit/${item._id}`} className="link" style={{color:"white"}}>Update</Link></DataUpdateButton>
            ...
            <Form onSubmit={onSubmit}>
                <FormGroup className="menupoints">
                  <Label htmlFor={`${uniqueId} lis`}>Menüpunkte Programm</Label>
...
export default DropdownProgramm