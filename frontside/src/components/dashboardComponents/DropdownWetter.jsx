...
import { createDropdownWetter, 
        getAllDropdownWetter, 
        deleteDropdownWetter, 
        reset } from "../../features/dropdown/wetterSlice";
...
const DropdownWetter = () => {
  //bring in
  const dispatch = useDispatch();
  const {dropdownWetter, isLoading, isError, message} = useSelector((state)=>state.dropdownWetter);
...
//bring out
  const [formdata, setFormdata] = useState(
    ...
    const onSubmit = async (e)=>{
      ...
      const dropdownWetterData = new FormData();
      dropdownWetterData.append("lis",JSON.stringify(formdata.lis));
      dropdownWetterData.append("themen",JSON.stringify(formdata.themen));
      dropdownWetterData.append("videos",JSON.stringify(formdata.videos));
      filedata.iframe.forEach((file, index)=>{
        dropdownWetterData.append(`iframe${index}`, file);
      })
      await dispatch(createDropdownWetter(dropdownWetterData));
      dispatch(getAllDropdownWetter());
    }
    else{
    ...
    const handleDelete = async (id)=>{
      await dispatch(deleteDropdownWetter(id))
      dispatch(getAllDropdownWetter());
    }
    if(isLoading){
      return <Spinner/>
  ...
  return (
    <Container>
      <ToastContainer/>
            <CrudTitleHolder>
              <CrudTitle>Dropdown Menü Wetter</CrudTitle>
            </CrudTitleHolder>
            <DataWrapper>
            {dropdownWetter.length > 0 && <DataHolder>
                {dropdownWetter.map((item, index)=>(
                  <div key={index}>
                    ...
                    <DataButtonHolder>
                      <DataUpdateButton><Link to={`/dropdownwetteredit/${item._id}`}className="link" style={{color:"white"}}>Update</Link></DataUpdateButton>
            ...
            <Form onSubmit={onSubmit}>
                <FormGroup className="menupoints">
                  <Label htmlFor={`${uniqueId} lis`}>Menüpunkte Wetter</Label>
