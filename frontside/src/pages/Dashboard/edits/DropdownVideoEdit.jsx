...
import {reset, updateDropdownVideos, getDropdownVideos} from "../../../features/dropdown/videosSlice";
...
cursor:pointer;
`;
const DropdownVideoEdit = () => {
    const dispatch = useDispatch();
    const navigate = useNavigate();
    const { id } = useParams();
    const uniqueId = useId();
    const singleDropdownVideo = useSelector((state)=>state.dropdownVideos.singleDropdownVideo);
    const isLoading = useSelector((state)=>state.dropdownVideos.isLoading);
    const isError = useSelector((state)=>state.dropdownVideos.isError);
    const message = useSelector((state)=>state.dropdownVideos.message);
    ...
    useEffect(()=>{
            if(singleDropdownVideo){
                setFormdata({...singleDropdownVideo})
            }
    },[singleDropdownVideo]);
...
const handleVideoChange = (index, e)=>{
    ...
    console.log(lis, themen);
      const onSubmit = (e) => {
        ...
        const dropdownVideosData = {
            formData,
            id:id
        }
        dispatch(updateDropdownVideos(dropdownVideosData));
      }
  ...
  return (
    ...
    <ToastContainer/>
        <TitleHolder>
            <Title>Update DropdownVideo</Title>
        </TitleHolder>
        <ContentHolder>
...
export default DropdownVideoEdit