...
const SalesServices = () => {
    ...
    const allAdvertises = useSelector((state)=>state.advertises.advertises);
    const allInlineAdvertises = useSelector((state)=>state.inlineAdvertises.inlineAdvertises);
    ...
    useEffect(()=>{
      setFilteredVideos(videos.filter((item)=>item.ressort === "Sales & Services"))
  },[videos])
        //mobile
          ...
          <TitleHolder>
                ...
                <SectionWrapper>
                  <NewsWrapper>
                    <Articles>
                      <NewsHolder>
                        {allAdvertises.length > 0 && <DataHolder>
                          {
                            allAdvertises.map((item)=>(
                              <Link to={{pathname: `/advertiseArticles/${item._id}`}} className="link" style={{color:"black"}} key={item._id}>
                              <Section key={item._id}>
                                <ImageHolder>
                                  <MainnewsImage src={item.img} alt={item.theme} title={item.theme}/>
                                </ImageHolder>
                                <ContentHolder>
                                  <MainnewsRessort>{item.adv}</MainnewsRessort> 
                      ...
                      <InlineNewsHolder>
                      {allInlineAdvertises.length > 0 && <DataHolder>
                          {
                            allInlineAdvertises.map((item)=>(
                              <Link to={{pathname: `/inlineAdvertiseArticles/${item._id}`}} className="link" style={{color:"black"}} key={item._id}>
                              <InlineSection key={item._id}>
                                <ImageHolder>
                                  <InlineImage src={item.img} alt={item.theme} title={item.theme}/>
                                </ImageHolder>
                                <InlineContentHolder>
                                  <InlineRessort>{item.adv}</InlineRessort>
