...
const DropdownVideos = require("../models/DropdownVideos");
...
router.post("/", upload.fields([{name:"iframe0", maxCount:1},{name:"iframe1", maxCount:1},{name:"iframe2", maxCount:1}]), verifyTokenAndAdmin, async (req, res) => {
  try {
    ...
    const newVideos = new DropdownVideos({
...
router.put(
    ...
    verifyTokenAndAdmin,
    async (req, res) => {
      let currentVideos = await DropdownVideos.findById(req.params.id);
      const files = req.files;
      //at first I store the currentVideos in uploadedFiles
      const uploadedFiles = currentVideos.videos.map(video => ({
      ...
      try {
        ...
        const updatedVideos = await DropdownVideos.findByIdAndUpdate(
          req.params.id,
          updatedVideosData,
          { new: true }
        );
...
router.delete("/:id", verifyTokenAndAdmin, async (req,res)=>{
    try{
        await DropdownVideos.findByIdAndDelete(req.params.id);
...
router.get("/find/:id", async (req, res)=>{
    try{
        const getVideos = await DropdownVideos.findById(req.params.id);
...
router.get("/find", async (req, res)=>{
    try{
        const getAllVideos = await DropdownVideos.find();
