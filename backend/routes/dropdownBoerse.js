...
const DropdownBoerse = require("../models/DropdownBoerse");
...
router.post("/", upload.fields([{name:"iframe0", maxCount:1},{name:"iframe1", maxCount:1},{name:"iframe2", maxCount:1}]), verifyTokenAndAdmin, async (req, res) => {
    try {
      ...
      const newBoerse = new DropdownBoerse({
        lis: JSON.parse(req.body.lis).split(','),
        videos,
        themen: JSON.parse(req.body.themen).split(','),
      });
      const savedBoerse = await newBoerse.save()
      res.status(200).json(savedBoerse)
    } catch (error) {
...
router.put(
  ...
  verifyTokenAndAdmin,
  async (req, res) => {
    let currentBoerse = await DropdownBoerse.findById(req.params.id);
    const files = req.files;
    //at first I store the currentVideos in uploadedFiles
    const uploadedFiles = currentBoerse.videos.map(video => ({
    ...
    try {
      ...
      if (req.body.themen) {
        ...
        themen = currentBoerse.themen;
      }
      const updatedBoerseData = {
        lis,
        videos,
        themen,
      };
      await new Promise((resolve) => setTimeout(resolve, 1000));
      const updatedBoerse = await DropdownBoerse.findByIdAndUpdate(
        req.params.id,
        updatedBoerseData,
        { new: true }
      );
      res.status(200).json(updatedBoerse);
    } catch (error) {
      res.status(404);
...
router.delete("/:id", verifyTokenAndAdmin, async (req,res)=>{
    try{
        await DropdownBoerse.findByIdAndDelete(req.params.id);
...
router.get("/find/:id", async (req, res)=>{
    try{
        const getBoerse = await DropdownBoerse.findById(req.params.id);
        res.status(200).json(getBoerse);
    } catch(error){
        res.status(404)
        throw new Error("Nicht gefunden");
    }
...
router.get("/find/", async (req, res)=>{
    try{
        const getAllBoerse = await DropdownBoerse.find();
        res.status(200).json(getAllBoerse);
    } catch(error){
        res.status(404)
        throw new Error("Nicht gefunden");
    }
