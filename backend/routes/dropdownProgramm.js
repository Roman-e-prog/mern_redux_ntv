...
const DropdownProgramm = require("../models/DropdownProgramm");
...
router.post("/", upload.fields([{name:"iframe0", maxCount:1},{name:"iframe1", maxCount:1},{name:"iframe2", maxCount:1}]), verifyTokenAndAdmin, async (req, res) => {
  try {
    ...
    const newProgramm = new DropdownProgramm({
      lis: JSON.parse(req.body.lis).split(','),
      videos,
      themen: JSON.parse(req.body.themen).split(','),
    });
    const savedProgramm = await newProgramm.save()
    res.status(200).json(savedProgramm)
  } catch (error) {
...
router.put(
    ...
    verifyTokenAndAdmin,
    async (req, res) => {
      let currentProgramm = await DropdownProgramm.findById(req.params.id);
      const files = req.files;
      //at first I store the currentVideos in uploadedFiles
      const uploadedFiles = currentProgramm.videos.map(video => ({
      ...
      try {
        ...
        if (req.body.themen) {
          ...
          themen = currentProgramm.themen;
        }
        const updatedProgrammData = {
          lis,
          videos,
          themen,
        };
        await new Promise((resolve) => setTimeout(resolve, 1000));
        const updatedProgramm = await DropdownProgramm.findByIdAndUpdate(
          req.params.id,
          updatedProgrammData,
          { new: true }
        );
        res.status(200).json(updatedProgramm);
      } catch (error) {
        res.status(404);
...
router.delete("/:id", verifyTokenAndAdmin, async (req,res)=>{
    try{
        await DropdownProgramm.findByIdAndDelete(req.params.id);
...
router.get("/find/:id", async (req, res)=>{
    try{
        const getProgramm = await DropdownProgramm.findById(req.params.id);
        res.status(200).json(getProgramm);
    } catch(error){
        res.status(404)
        throw new Error("Nicht gefunden");
    }
...
router.get("/find/", async (req, res)=>{
    try{
        const getAllProgramm = await DropdownProgramm.find();
        res.status(200).json(getAllProgramm);
    } catch(error){
        res.status(404)
        throw new Error("Nicht gefunden");
    }
